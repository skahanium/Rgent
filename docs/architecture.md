...
- **渲染进程：** CodeMirror 6 即时渲染（文档是 Markdown 字符串）、文件树、快捷键、把流式结果画进正文或幕后区块。公式 / Mermaid / 表格 / 图 / wikilink / callout / 嵌入用 viewport 内 widget，懒加载。无 Node、不能直接碰盘。
- **主进程：** 打开库、读写文件、ACL、索引、窗口、单实例、系统凭据库。`AgentHost` 住在这里。
- **AgentHost：** 一次任务的 **Harness**（目标 → 工具 → 观察、硬刹车、错误回喂、完成核对）。模式（改写 / 问答 / 强制检索）、注入 prompt-only skills、调用 AI SDK、把 MCP 与库内工具接到同一张白名单上。模型看不到原始文件系统。跨任务的 **外循环**（自触发、无人值守接力）v0 不做。
...
2. 主进程进入 Harness 循环：判定模式（改写走 `editor_*`；跨文件走 `vault_*`；命中必须遵循则先检索）；每轮工具结果经 ACL 后回喂模型，直到完成核对、触顶刹车、或交给用户。
3. 工具结果经 ACL。禁区、规则目录、必须遵循上的写，直接失败并把原因写给模型（例如改为新建笔记）。
4. 流式 token 回到渲染进程：替换选区、插入段落、或写入幕后区块。轨迹可复盘。

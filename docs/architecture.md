...
6. 回车关闭输入态并开始生成；Esc、切 tab、关窗取消生成。未采纳块留在当时那篇里，并记原因。
...
| 笔记路径与冲突写盘 | IO | 笔记文件及父目录若是符号链接，树仅按普通文件显示，读写、索引和监听不跟随。`noteRead` 给全文及内容修订值，`noteWrite` 带预期修订值；同篇应用内串行检查，同目录临时文件原子替换，冲突交给人选磁盘或窗口稿。 |
| 权限名单损坏时关闭 AI 出口 | 主进程 | 名单缺失是空名单；已有名单损坏、无效或不可读是 `invalid`。人读写搜继续，树上提示修复，未来模型出口拒绝。名单写入只接受有效文件夹与三档值，原子替换。 |
...
| 退出不丢稿 | 壳 | 关窗先 `flushRequest`。`flushDone({ ok: true })` 才关。失败不关窗。超时只在渲染进程已死时关。`vault.dispose()` 在 `will-quit`，不在 `before-quit`（退出 flush 还要走 `noteWrite`）。 |

...
**身份标记。** 落盘写法已拍板（`decisions.md` §记录即正文），本阶段按它做：身份标记的读写与画布区分、账本只读回顾。不开放 Host，不碰模型与工具。最容易做偏的是「未采纳块不能改字」——不许用整篇只读冒充。
...
已知残余（不挡本阶段）：账本回顾现在只能显示手写的账本原文，**按次按日期分场要等写入方**（Host 阶段）；四五个动作只有 chip 这一条入口，没有键盘快捷键。`planWidgets` 现在按整篇规划（StateField 拿不到视口），实测 57 KB / 1000 个标记时每次约 0.4 ms，大库压测时要重新看这一处。
...
评审点名、尚未修的（记在这里，别丢）：`conflictOpen` 是全局标志，一次冲突弹窗未结时后来的外部改动通知会被丢掉；退出期间若同时来两条冲突路径会叠弹窗；`scripts/check-docs.mjs` 只查链接与阶段串，不查锚点是否存在；渲染进程应用外部快照时没有 revision 守卫（窄回退窗口，见 [架构强制点](architecture.md) 的写盘一行）。

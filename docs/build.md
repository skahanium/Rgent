...
权限名单、IPC、树徽章和静态路径防护已接线，`modelTierFor` 目前只在测试中调用，没有实际模型出口。退出保存失败的选择路径和 CI 构建产物检查已接线，状态流程测试、本机窗口启动及 macOS、Windows 远端 CI 已通过；失败弹窗仍待实际交互复核。阶段尚未交付，剩余门槛是：只设 `X/` 时 `X.md` 不跟档及平局规则的定向断言；笔记和名单读取的并发换链、Windows 路径别名的失败关闭。

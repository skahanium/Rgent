...
- 并发换链时笔记与权限名单不得从库外读；Windows 同一路径的大小写和 Unicode 别名不得绕过禁区。无法安全判定则 AI 失败关闭。
- 保存失败时关窗与 `Cmd+Q` 均能让人继续编辑、重试，或明确放弃修改并退出。
- 相对文档链接和当前阶段检查；macOS、Windows CI 测试、构建，以及构建后 `out/main/index.js` 存在、`node --check out/preload/index.cjs` 通过。语法检查只能拦已知的 ESM 回归，不能代替窗口启动。
- 树上禁止 / 必须遵循打标，可参考不打。
- 文件夹右键改档后能再读回来。
...
权限名单、IPC、树徽章和静态路径防护已接线，`modelTierFor` 目前只在测试中调用，没有实际模型出口。退出保存失败的选择路径和 CI 构建产物检查已接线，状态流程测试与本机窗口启动已通过；失败弹窗仍待实际交互复核。阶段尚未交付，剩余门槛是：只设 `X/` 时 `X.md` 不跟档及平局规则的定向断言；笔记和名单读取的并发换链、Windows 路径别名的失败关闭；macOS、Windows 远端 CI 结果。

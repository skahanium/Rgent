...
权限名单、IPC、树徽章和静态路径防护已接线，`modelTierFor` 目前只在测试中调用，没有实际模型出口。阶段尚未交付，剩余门槛是：退出保存失败的选择路径；只设 `X/` 时 `X.md` 不跟档及平局规则的定向断言；笔记和名单读取的并发换链、Windows 路径别名的失败关闭；CI 对构建后 preload 的语法检查与 macOS、Windows 远端结果。CI 构建成功不能代替构建后实际窗口启动。

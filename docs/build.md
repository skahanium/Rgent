...
- **读标记。** 注释在管线里被消费掉、不进块清单；被它标的块带身份进 `DocIndex`。缺省即人写的。
- **画布区分。** 标记行藏起来，未采纳的 AI 块与口令各有一种样子；「采纳」= 删掉那枚标记（正文一字不改），「丢弃」= 连同它标的块一起删。
- **未采纳块不能改字。** 画布拒绝落在 AI 块范围内的编辑，先采纳再改。这是本阶段最容易做偏的一处：不要用「只读整篇」冒充它。
- **拖动是搬家。** 只允许拖到段落之间；拖动不改身份，不改块编号（编号本来就只活在本次打开期间）。
- **写盘不变形。** 标记跟正文落在同一个文件里，不引入第二个文件；`composeSource` 的往返要覆盖带标记的正文。
- **账本只读回顾 tab。** 入口在当前笔记标题旁边，临时 tab、只读、关掉就走、不占右侧反链（围栏 §账本）。**注意：现在没有任何代码写账本**，所以这块只能靠手写的账本内容验证；不要为了让它「看起来有内容」而去实现写入方——写入方是 Host 阶段的事。
...
- 口令块与 AI 块在画布上是两种样子；标记行看不见但删得掉（采纳）。
- 未采纳块内无法通过键盘或粘贴改字；采纳后立即可改。
- 拖动只落在段落之间，且不改身份。
...
- 画布区分：机器语法看不见，留一枚 chip；未采纳的 AI 块与口令各一种行样式。采纳 / 丢弃 / 上移 / 下移四个动作都在 chip 上，动作的文本变更在计划层一次算完（`test/markdown/view-plan.test.ts`、`test/markdown/identity-edit.test.ts`）。
- 未采纳块不能改字：纯判定 + `transactionFilter`，只拦用户输入与删除（`test/markdown/identity-lock.test.ts`）。
- 拖动是搬家：v0 用 chip 上的上移 / 下移，标记跟着它标的块走；指针拖拽手感留给交互像素那一刀。
...
已知残余（不挡本阶段）：账本回顾现在只能显示手写的账本原文，**按次按日期分场要等写入方**（Host 阶段）；那三个动作目前只有 chip 这一条入口，没有键盘快捷键。

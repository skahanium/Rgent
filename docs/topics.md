...
门禁（tierFor）
  ...
  → 内置工具：读库、补任务发起篇、建、改名、挪、删；搜库（可引用片段，方法未锁）
  → 技能
  → MCP（仅代转）
  → 联网搜 / 抓页
```
...
本阶段先修笔记同目录原子替换、内容修订值和同篇应用内串行写；外部修改冲突交给人选磁盘或窗口稿。退出保存失败还须给出继续编辑、重试或明确放弃退出的路径。流式回画布走现有 IPC；未来取消用 `AbortSignal` 覆盖 Esc、停止按钮和关窗，切 tab 仅结束输入交互，不发取消信号。

...
- **未采纳块不能改字**：CM6 没有「局部只读」原语，用 `EditorState.transactionFilter` 拒绝与 AI 块范围相交的变更（打字、粘贴、删词都算）。**不许**用 `editable = false` 或整篇 `readOnly` 冒充——那会连人的正文一起锁住。
- **拖动是搬家**：只允许落在段落之间，落点按块边界判定，不按像素。v0 用「上移 / 下移」这类显式命令即可满足「拖只换位置」；指针拖拽的手感等交互像素那一刀再定。
- **账本只读 tab**：临时 tab、只读视图，内容直接来自 `partitionSource` 的 `ledger` 段，不新增第二份解析，也不占右侧反链。

### 实现路线

先管线（读标记 → `DocIndex` 带身份 → `composeSource` 往返不变形），再画布（藏标记行、两种样子、采纳 / 丢弃、拒改），最后账本只读 tab。

账本只读 tab 现在只能靠手写的账本内容验证：**写入方属于 Host 阶段**（围栏把只读回顾排在写入之前）。不要为了让它「有内容」而先实现写入方。
